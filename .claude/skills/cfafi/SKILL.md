...
description: >
  ...
  routes", "cf-whoami", "cfafi whoami", "cfafi zones list",
  "inventory agentirc", "verify the cloudflare token".
---
...
[`cfafi-write`](../cfafi-write/SKILL.md) skill — it lives
...
Install: `uv tool install cfafi`. Run `cfafi learn` for the full agent
prompt; `cfafi explain <path>` for per-verb docs.
...
errors. `cfafi whoami` does NOT list the token's granted scopes —
...
2 (env), 3 (auth), 4 (API). Run `cfafi learn` for the full agent
prompt, or `cfafi explain <path>` for per-verb docs.
...
bash .claude/skills/cfafi/scripts/cf-dns.sh culture.dev

# What Workers are behind culture.dev?
bash .claude/skills/cfafi/scripts/cf-workers-routes.sh | grep culture.dev

# What Pages projects point at culture.dev?
bash .claude/skills/cfafi/scripts/cf-pages.sh --json | jq '.result[] | select(.domains | any(test("culture.dev")))'
```
...
bash .claude/skills/cfafi/scripts/cf-pages.sh | grep -i agentirc

# Dump all deployments for that project
bash .claude/skills/cfafi/scripts/cf-pages.sh <project-name>

# Check DNS records still attached to agentirc.dev
bash .claude/skills/cfafi/scripts/cf-dns.sh agentirc.dev

# Check whether any Workers routes still point at agentirc.dev
bash .claude/skills/cfafi/scripts/cf-workers-routes.sh | grep agentirc.dev
```
...
- **Single-object data** (only `cfafi whoami` / `cf-whoami` today) → key-value list (`- **key:** value`).

`--json` bypasses all formatting:

```sh
# Pipe JSON into jq
bash .claude/skills/cfafi/scripts/cf-zones.sh --json | jq '.result[].name'

# Check how many DNS records culture.dev has
bash .claude/skills/cfafi/scripts/cf-dns.sh culture.dev --json | jq '.result | length'
```
...
`CF_PAGE_SIZE=25 bash .claude/skills/cfafi/scripts/cf-zones.sh`.
...
- **Write operations.** No create / update / delete. Added in a
  later phase with the same `.claude/skills/cfafi/scripts/`
  layout (new `cf-*-create.sh` / `cf-*-delete.sh` scripts).
- **Scope enumeration.** `cfafi whoami` / `cf-whoami.sh` reports token
...
- `../cfafi-write/references/cf-api-gotchas.md` — consolidated
