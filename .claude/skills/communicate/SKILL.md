...
description: >
  ...
  GitHub issues on sibling repos, and send live messages to Culture
  mesh channels. Use when the next step lives outside cultureflare
  (a brief for a sibling-repo agent, a status ping for a Culture
  channel). Issue posts auto-sign with `- cultureflare (Claude)`;
  mesh messages are unsigned (the IRC nick is the speaker). Not for
  in-cultureflare issues — use `gh issue create` or the `cicd` skill
  for those. Vendored from steward 0.8.0.
---

# Communicate (Cross-Repo + Mesh)

cultureflare's job is to keep CloudFlare state aligned across the
AgentCulture mesh; that surfaces in two distinct channels:
...
|---------|-----------|-----|
| GitHub issues | `- cultureflare (Claude)` (auto-appended by `post-issue.sh`) | Cross-repo audit trail — readers can tell at a glance which sibling and that it came from an AI. |
| Culture mesh | none — unsigned | The IRC nick already identifies the speaker. A trailing `- cultureflare (Claude)` would be visual noise that the nick already supplies. |
...
`- cultureflare (Claude)` is auto-appended at the end of the body.
...
| Script | Purpose |
|--------|---------|
| `scripts/post-issue.sh` | Create a new issue on a target repo. Auto-signs `- cultureflare (Claude)`. |
...
- Post a brief that says "see cultureflare's plan" without inlining the
  content. Briefs must be self-contained.
- Skip the issue signature. The script enforces it; do not introduce a
  `--no-signature` flag.
- Sign mesh messages with `- cultureflare (Claude)`. The nick already
  says who you are.
- Use this skill for in-cultureflare issues — use `gh issue create` or
  the `cicd` skill instead.
- Manually type `- cultureflare (Claude)` at the end of an issue body
  — the script appends it. Manual typing creates double-signatures
  when the script is later refactored.
